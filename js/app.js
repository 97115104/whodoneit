...
async function handleCreateAttestation() {
    ...
    if (signMethod === 'metamask') {
        ...
        // Unsigned — record the originating tool so attest.ink can display it
        attestationData.signature = {
            type: 'whodoneit',
            value: 'unsigned',
            signer: 'whodoneit'
    ...
    if (signMethod === 'metamask' && signerAddress) {
        ...
        signedInfo = '<br>• Signature: Local key (HMAC-SHA256, password-derived)';
    } else {
        signedLabel = '<span style="font-size:11px;opacity:0.8;"> — Generated by whodoneit</span>';
        signedInfo = '<br>• Signature: Generated by whodoneit (unsigned)';
    }
